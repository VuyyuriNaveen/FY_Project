...
# Main function to predict using TensorFlow model
    ...
    if app_mode == "Home":
        ...
        if test_image is not None:
            ...
            if st.button("Predict"):
                ...
                disease_info = pd.read_csv("disease_info.csv", encoding='latin1')
                supplement_info=pd.read_csv("supplement_info.csv", encoding='latin1')

                # Assume pred is the index for the prediction
                pred = result_index  # You need to replace this with your actual prediction index

                if pred < len(disease_info) and pred < len(supplement_info):
                    ...
                    st.write(prevent)
                    st.header("Supplement Information")
                    st.subheader(supplement_name)
                    st.image(supplement_image_url, caption="Supplement Image", use_column_width=True)
                    st.write("**Buy Link:**")
                    st.write(supplement_buy_link)
